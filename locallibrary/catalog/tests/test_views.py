...
class LoanedBookInstancesByUserListViewTest(TestCase):
    def setUp(self):
        ...
        # Create a book
        test_author = Author.objects.create(first_name='Dominique',
                                            last_name='Rousseau')
        ...
        for book_copy in range(number_of_book_copies):
            dat = timezone.localtime() + datetime.timedelta(days=book_copy % 5)
            return_date = dat
...
class AuthorCreateViewTest(TestCase):
    ...
    def test_author_create_view_get_with_permission(self):
    ...
    def test_author_create_view_get_without_permission(self):
        User.objects.create_user(
            username='no_perm_user', password='12345'
        )
        self.client.login(username='no_perm_user', password='12345')

        response = self.client.get(reverse('author-create'))

        self.assertEqual(response.status_code, 403)

    def test_author_create_view_get_anonymous(self):
        response = self.client.get(reverse('author-create'))

        self.assertEqual(response.status_code, 302)

    def test_author_create_view_post_valid(self):
        self.client.login(username='test_user', password='some_password')
        ...
        self.assertEqual(response.status_code, 302)
        self.assertTrue(Author.objects.filter(first_name='Isaac').exists())

    def test_author_create_view_post_without_permission(self):
        User.objects.create_user(
            username='no_perm_user', password='12345'
        )
        self.client.login(username='no_perm_user', password='12345')

        data = {'first_name': 'Autor ilegal'}

        response = self.client.post(reverse('author-create'), data)

        self.assertEqual(response.status_code, 403)
        self.assertFalse(Author.objects.filter(first_name='Autorilegal').
                         exists())
    ...
    def test_form_date_of_death_initially_set_to_expected_date(self):
        ...
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['form'].initial.get('date_of_death'),
                         '11/11/2023')
    ...
    def test_redirects_to_detail_view_on_success(self):
        ...
        author = Author.objects.filter(first_name='Javier',
                                       last_name='Rubio').first()
